...
export function TestFinalForm({ testFinal }: { testFinal: TestFinal | null }) {
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-6">
      <div className="mb-4 flex items-center justify-between">
        <div>
          <h2 className="text-base font-medium text-slate-900">Test general de evaluare</h2>
      ...
      <form action={formAction} className="flex flex-col gap-4">
        {testFinal && <input type="hidden" name="id" value={testFinal.id} />}
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="mb-1 block text-sm text-slate-600">Titlu test</label>
            <input
              name="titlu"
              defaultValue={testFinal?.titlu ?? "Test general de evaluare SMAM"}
              required
          ...
          <div>
            ...
            <input
              ...
              type="number"
              min={1}
              defaultValue={testFinal?.nrIntrebari ?? 20}
              required
              className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm text-slate-900"
            />
          </div>
        ...
        <div className="flex items-center gap-3">
          ...
          >
            <Save size={14} /> {pending ? "Se salveaza..." : testFinal ? "Actualizeaza" : "Creeaza testul general"}
          </button>
          {testFinal && (
